...
// @updateURL      https://github.com/MysticJay/ZasoItems.CE/raw/master/link-prolongation.meta.js
...
// 0.1.0 Original script
	...
	window.plugin.linkProlongation.data.setDrawType = function(number){
        var number = (parseInt(number) === NaN)? 5 : parseInt(number);
...
//            window.plugin.linkProlongation.obj.points['p3'] = p3;
    ...
    window.plugin.linkProlongation.action.drawFavoriteType = function(){
        if(window.plugin.drawToolsPlus === undefined){
            window.plugin.linkProlongation.dialog.openBoxMessage('<i class="fa fa-exclamation-triangle"></i> <i>Draw Tools</i> and <i>Draw Tools Plus</i> plugins needed to draw.');
            return false;
        }
...
//        var portalsList = JSON.parse(window.plugin.bookmarks.KEY_STORAGE);
        ...
        for(var idFolders in list) {
            ...
            for(var idBkmrk in fold) {
                ...
                // Create the bookmark
                elementTemp += '<a class="bkmrk" id="'+idBkmrk+'" onclick="window.plugin.linkProlongation.bookmarks.clickOnListItem(this);return false" data-latlng="['+latlng+']">'+label+'</a>';
            }
            elementTemp += '</div></div>';
...
//                + '</div>'
    ...
    window.plugin.linkProlongation.dialog.noBookmarksPlugin = function(){
        if(window.plugin.bookmarks === undefined){
            window.plugin.linkProlongation.dialog.openBoxMessage('<i class="fa fa-exclamation-triangle"></i> <i>Bookmarks for Maps and Portals</i> plugin needed for this features.');
            return false;
        }
        return true;
    }
...
//                html += '<label>Portal 1 (P1)</label>';
                html += '<button class="p1 getCurr" onclick="window.plugin.linkProlongation.data.setP1fromPortal();return false;" title="Get LatLng from selected portal"><i class="fa fa-mouse-pointer"></i></button>';
                html += '<button class="p1 getBkmrk" onclick="window.plugin.linkProlongation.dialog.openBookmarkChooser(1);return false;" title="Get LatLng from a your portal bookmarked"><i class="fa fa-bookmark"></i></button>';
                ...
                html += '</span>';
                html += '<button class="p1 save" onclick="window.plugin.linkProlongation.data.setPoint_1([$(this).prev(\'.latlng\').children(\'input\').eq(0).val(), $(this).prev(\'.latlng\').children(\'input\').eq(1).val()]);return false;" title="Save"><i class="fa fa-save"></i></button>';
            html += '</div>';
...
//                html += '<label>Portal 2 (P2)</label>';
                html += '<button class="p2 getCurr" onclick="window.plugin.linkProlongation.data.setP2fromPortal();return false;" title="Get LatLng from selected portal"><i class="fa fa-mouse-pointer"></i></button>';
                html += '<button class="p2 getBkmrk" onclick="window.plugin.linkProlongation.dialog.openBookmarkChooser(2);return false;" title="Get LatLng from a your portal bookmarked"><i class="fa fa-bookmark"></i></button>';
                ...
                html += '</span>';
                html += '<button class="p2 save" onclick="window.plugin.linkProlongation.data.setPoint_2([$(this).prev(\'.latlng\').children(\'input\').eq(0).val(), $(this).prev(\'.latlng\').children(\'input\').eq(1).val()]);return false;" title="Save"><i class="fa fa-save"></i></button>';
            html += '</div>';
...
//        html += '</div>';
	...
	window.plugin.linkProlongation.dialog.getHtmlSettingsBox = function(){
        ...
        function getHtmlLabel(arr, val){
            ...
            check = (currDrawType === val)? 'checked' : '';
        ...
        html += '<div class="row" style="text-align:center;margin-bottom:0px;">';
            ...
            html += '<button class="dist save" onclick="window.plugin.linkProlongation.data.setDist($(this).prev(\'input\').val());return false;" title="Save"><i class="fa fa-save"></i></button>';
        html += '</div>';
...
//        html += '<hr class="clearfix" />';
        ...
        html += '<div class="row">'
            ...
            html += '<label>'
                html += '<input onchange="window.plugin.linkProlongation.action.setDrawColorStatus($(this).prop(\'checked\'));" type="checkbox" '+((currDrawColor.enabled)? 'checked': '')+'/>'
            ...
            html += '<small class="row">NB: for the latest 4 <i>draw types</i> the custom draw color will be applicated only for <span class="overline">aA</span> and <span class="overline">Bb</span>.</small>'
        html += '</div>';
        }
    ...
    window.plugin.linkProlongation.thirdParty = function(){
        ...
        // Thanks to TheSned (https://github.com/TheSned/ingress-intel-total-conversion/blob/e23ea7bafb4610911b9a8772700e40b2e0836873/plugins/extend-poly-lines.user.js#L38)
